...
function adapter_video(dato=[]){
		   ...
		   var domain="";
			...
			return `
				<tr>
					<td>${dato['titulo']}</td>
					<td><img src='${domain}/${dato.ruta_imagen}' style='height:50px;width:50px;margin-top:5px;' class='img-responsive'></td>
					<td><button
						class='btn  btn-success'  style='float:right;background:#FF3399;
			 			border-color:#FF3399; float:right;margin-top:5px;' onclick='eliminar_video(${dato.id_post})'>Delete</button></td>
					<td><button class='btn btn-success' style='background:#FF3399; border-color:#FF3399;margin-top:5px;' onclick='editar_video(${dato.id_post})'>Edit</button></td>
				</tr>
			`;
...
$('#video').click(function(){
	...
	$.ajax({
		...
		var  videos=`<input type='text' class='form-control' id='video_search'>
		<div style='overflow:scroll; height:400px;' id='video_panel'>`;
...
$('#Upload_video_s').click(function(){
	...
	var load_video =`<br><br><form method="post"  enctype="multipart/form-data" id="myForm" action="https://files.videosegg.com/guardar_video.php">
